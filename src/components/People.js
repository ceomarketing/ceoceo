import React, { Component } from "react";
import styles from '../css/people.module.css';

class People extends Component {
  render() {
    return (
    <div className="people page-content">

      <div className="container-sm">
        <div className="page-intro">
          <div className="container">
          <h1 className="page-title">华人创业者协会</h1>
          <h1 className="page-title">Chinese Entrepreneur Organziation</h1>
          <p className="lead">
          CEO organizes in-depth small-group entrepreneurial talks, brainstorm sessions, etc.
          </p>
          </div>  
        </div>
      </div>

      <div className="page=content container-sm">
        <div className="section highlight" id="team-section">
          <div className="container">
            <h2>Executive Team</h2>
                <div className="row about-organisers">
                <div className="col-sm-3 col-md-2"/>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-xty.jpg" alt="Co-President" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">谢天宇</a>
                          </h4>
                          <p class="organiser_twitter">人类学博士</p>
                          <p class="mv-o">Co-President</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-april.jpg" alt="CEO leaders - XXX" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                            <a href="" target="_blank">王博闻(April）</a>
                          </h4>
                          <p class="organiser_twitter">MBA</p>
                          <p class="mv-o">Co-President</p>
                        </div>
                      </div>
                    </div>
                    <div className="col-sm-3 col-md-2"/>
                  </div>
                </div>
                <div className="row about-organisers">
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-eric.jpg" alt="CFO" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Eric Zhang</a>
                          </h4>
                          <p class="organiser_twitter">MBA</p>
                          <p class="mv-o">CFO</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-alice.jpg" alt="VP(Marketing)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">郑怡（Alice）</a>
                          </h4>
                          <p class="organiser_twitter">Product Design & Computer Science</p>
                          <p class="mv-o">VP（Marketing）</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-yanyan.jpg" alt="VP(Operations)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Yanyan Tong</a>
                          </h4>
                          <p class="organiser_twitter">MS in Computer Science</p>
                          <p class="mv-o">VP (Operations)</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="row about-organisers">
                <div className="col-sm-3 col-md-2"/>
                  <div className="col-sm-3 col-sm-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-zx.jpg" alt="CFO" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">张霄</a>
                          </h4>
                          <p class="organiser_twitter">Applied Physics PhD</p>
                          <p class="mv-o">VP (Innovation Mixer)</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-sm-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/organiser-fx.jpg" alt="VP(Marketing)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">沙扬贺</a>
                          </h4>
                          <p class="organiser_twitter">MS in East Asian Studies</p>
                          <p class="mv-o">VP (BD)</p>
                        </div>
                      </div>
                    </div>
                    <div className="col-sm-3 col-md-2"/>
                  </div>
                </div>
              </div>
        </div>
        <div className="section highlight" id="guest-section">
          <div className="container">
           <h2>Guest & Speakers</h2>
                <div className="row about-organisers">
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="CFO" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="VP(Marketing)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="VP(Operations)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
            </div>
        </div>
        <div className="section highlight" id="community-section">
          <div className="container">
            <h2>Community</h2>
            <div className="row about-organisers">
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="CFO" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="VP(Marketing)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="col-sm-3 col-md-4">
                    <div className="organiser">
                      <div className="organiser_header">
                        <div className="img-wrapper">
                          <img src="./images/ceo_logo.png" alt="VP(Operations)" width="80" height="80" className="img-circle"></img>
                        </div>
                        <div className="text-wrapper">
                          <h4 class="organiser_name">
                              <a href="" target="_blank">Placeholder</a>
                          </h4>
                          <p class="organiser_twitter">Placeholder</p>
                          <p class="mv-o">Placeholder</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>  
          </div>
        </div>
      </div>
    </div> 
    );
  }
}
 
export default People;


